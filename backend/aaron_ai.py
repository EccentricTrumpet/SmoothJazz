...
class AaronAI():
    def __init__(self, game: Game, player: Player) -> None:
        ...
        self.__player_name = player.player_name
        self.__game = game
    ...
    def __try_declare_trump(self, gameProto: GameProto) -> None:
        ...
        if cards_to_play:
            success, err_str = self.__game.play(self.__player_name, cards_to_play)
            logging.info(f'Aaron AI {self.__player_name} tries to declare trump as {cards_to_play[0].rank}. Success: {success}; Error: {err_str}')

    async def play(self):
        for gameProto in self.__player.update_stream():
            if gameProto.HasField('card_dealt_update'):
                self.__card_count += 1
                logging.info(f'Card count: {self.__card_count} - {gameProto.card_dealt_update.player_name} - {self.__player_name} - {gameProto.trump_player_name}')
                if gameProto.card_dealt_update.player_name == self.__player_name:
                    self.__try_declare_trump(gameProto)
            if self.__card_count == 100 and gameProto.trump_player_name == self.__player_name:
                time.sleep(1)
                self.__game.drawCards(self.__player_name)
            if self.__card_count == 108 and gameProto.trump_player_name == self.__player_name:
                time.sleep(1)
                cards_to_play = []
                ...
                success, err_str = self.__game.play(self.__player_name, cards_to_play)
                logging.info(f'Aaron AI {self.__player_name} plays {cards_to_play}. Success: {success}; Error: {err_str}')
