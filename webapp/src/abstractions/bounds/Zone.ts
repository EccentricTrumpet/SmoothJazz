...
export class Zone {
    ...
    public midSet(size = this.size, v = Vector.Origin) {
        return new Zone(this.center().add(v).add(size.vector().scale(-0.5)), size);
    }
    ...
    public inSet(v = Vector.Origin, size = this.size) {
    ...
    public outSet(v = Vector.Origin, size = this.size) {
        let x = v.x, y = v.y, mid = this.center();
